...
// --- [新規追加] IoTデータの所有権登録 ---
export async function registerIoTData(registry, from, cid) {
    return await registry.methods
        .registerIoTData(cid)
        .send({ from, gas: 300000 });
}
...
// --- [新規追加] アクセスポリシー設定 ---
export async function setAccessPolicy(registry, from, cid, requiredAttribute) {
    return await registry.methods
        .setAccessPolicy(cid, requiredAttribute)
        .send({ from, gas: 300000 });
}

// --- [新規追加] ポリシー取り消し ---
export async function removeAccessPolicy(registry, from, cid) {
    return await registry.methods
        .removeAccessPolicy(cid)
        .send({ from, gas: 300000 });
}

// --- [新規追加] VC提示によるアクセス検証 ---
export async function verifyAndAccess(registry, from, cid, attribute, signature, issuerA) {
    return await registry.methods
        .verifyAndAccess(cid, attribute, signature, issuerA)
        .call({ from });
}